...
type CharacterService interface {
	ListByFaction(ctx context.Context, faction string, params pagination.CursorParams) (pagination.Page[models.CharacterResponse], error)
}
...
func (s *characterService) ListByFaction(ctx context.Context, faction string, params pagination.CursorParams) (pagination.Page[models.CharacterResponse], error) {
	...
	if hasMore {
		cursor, err := pagination.CursorFields{"id": rows[len(rows)-1].ID}.Encode()
		if err != nil {
			return pagination.Page[models.CharacterResponse]{}, err
		}
		nextCursor = &cursor
